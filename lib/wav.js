...
/**
 * Encoding → WAV header fields.
 *
 * This table exists so the container can never disagree with the requested
 * encoding. A PCM header (format code 1) written over companded audio — mulaw is
 * 7, alaw is 6 — produces a file that plays as noise, and nothing in the pipeline
 * would catch it: the byte count is right, the duration is right, and the report
 * looks complete.
 *
 * Only linear16 is supported, because that is the only encoding this tool
 * requests. Anything else throws rather than falling back to PCM, so adding an
 * encoding to the request forces a deliberate decision here instead of silently
 * mislabelling the output.
 */
const ENCODINGS = new Map([["linear16", { formatCode: 1, bitsPerSample: 16, bytesPerSample: 2 }]]);

export const DEFAULT_ENCODING = "linear16";
 ...
 */
export function toWav(samples, sampleRate, encoding = DEFAULT_ENCODING) {
  const fmt = ENCODINGS.get(encoding);
  if (!fmt) {
    throw new Error(
      `no WAV header mapping for encoding "${encoding}". Add it to ENCODINGS in lib/wav.js — ` +
        "do not fall back to PCM, which would mislabel the container and ship unplayable audio.",
    );
  }

  const dataBytes = samples.length * fmt.bytesPerSample;
  const header = Buffer.alloc(44);
  const channels = 1;
  const byteRate = sampleRate * channels * fmt.bytesPerSample;
  ...
  header.writeUInt16LE(fmt.formatCode, 20);
  header.writeUInt16LE(channels, 22);
  header.writeUInt32LE(sampleRate, 24);
  header.writeUInt32LE(byteRate, 28);
  header.writeUInt16LE(channels * fmt.bytesPerSample, 32); // block align
 ...
 */
export function process(pcm, sampleRate, encoding = DEFAULT_ENCODING) {
  const trim = trimLeadingSilence(pcm, sampleRate);
  const peak = measurePeak(trim.samples);
  return {
    wav: toWav(trim.samples, sampleRate, encoding),
